...
from .forms import SweetPostForm
...
class IndexView(ListView):
    ...
    # 投稿日時の降順で並べ替える
    queryset = SweetPost.objects.order_by('-posted_at')
    # 1ページに表示するレコードの件数
    paginate_by = 3
...
class CreateSweetView(CreateView):
    '''写真投稿ページのビュー
    ...
    form_class = SweetPostForm
    # レンダリングするテンプレート
    template_name = "post_photo.html"
    # フォームデータ登録完了後のリダイレクト先
    success_url = reverse_lazy('sweet:post_done')
...
class CategoryView(ListView):
    ...
    def get_queryset(self):
      ...
      categories = SweetPost.objects.filter(
...
class UserView(ListView):
    ...
    def get_queryset(self):
      ...
      # filter(フィールド名=id)で絞り込む
      user_list = SweetPost.objects.filter(
...
class MypageView(ListView):
    ...
    def get_queryset(self):
      ...
      queryset = SweetPost.objects.filter(
        user=self.request.user).order_by('-posted_at')
      # クエリによって取得されたレコードを返す
      return queryset
  
class SweetDeleteView(DeleteView):
    '''レコードの削除を行うビュー
    ...
    template_name ='sweet_delete.html'
    # 処理完了後にマイページにリダイレクト
    success_url = reverse_lazy('sweet:mypage')
